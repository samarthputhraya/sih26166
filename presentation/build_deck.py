...
Output   presentation/SIH26166_LunaXX_deck.pptx   (national round, v4 - 20 Sep 2026)
...
S2 = [
    ...
    ("Sun: one OHRC frame vs 25 NAC frames, azimuths 3–153° apart. Scale: to NAC ~1 m "
     "and Kaguya TC 7.4 m (29.6×, 3/4 accepted); LOLA 60 m: declared failure.",
     1, False, INK),
    # "Kaguya TC -> Kaguya MI": 1548 nm rows declared fallback / rows; its "Fallback vs the visible
    # band" table -> disagreement median px (m) range. "... IIRS": declared / verdict counts.
    ("Multi-modal: Kaguya TC (visible) to MI 1548 nm — LoFTR refused on 3/3 windows; the "
     "declared fallback lands 3.4–16.2 m from the visible-band registration of the same window "
     "(0.23–1.09 px on MI's 14.8 m grid). TC to Chandrayaan-2 IIRS (12×, 89 m): 10/11 refused, "
     "none registers.", 1, False, INK),
    ...
    # "Trust layer on real imagery: planted ...", the "under 10°" population: 0 m row = false
    # alarms; 3 m row; >= 5 m rows. The "132-174°" population (SAC's pairs): windows, 0 m, >= 5 m.
    # Two "0/N" figures in one sentence, the first a strength and the second a weakness: name the
    # second as the floor so it cannot be read the flattering way (claim-checker M5, v4).
    ("Planted wrong answers, 22 real OHRC/NAC windows (Sun azimuths under 10° apart): 0/44 "
     "false alarms; flagged 74% at 3 m, 100% from 5 m, and 0 of 352 at 1–2 m — the floor; "
     "8 SAC windows, Sun "
     "azimuths 132–174° apart: 0/16 false alarms, 84% at 5 m, 100% from 10 m. No matches means "
...
S3_LEFT = [
    ...
    ("Data: Chandrayaan-2 OHRC, TMC-2, IIRS (PRADAN) · LRO NAC · Kaguya TC and MI · LOLA.",
     1, False, INK),
    # "Runtime and match distribution": median seconds over the 74 °S OHRC -> NAC windows.
...
S4 = [
    ...
    # Trimmed to one line in v4 to make room for the tiling and sub-pixel clauses: the dropped
    # tail ("public data, open-source libraries; one command re-runs every number from a clean
    # commit") is already on slide 3, in Technologies and in the last Methodology bullet.
    ("Built and measured: 160 distinct ground windows, 8 instrument pairings.", 1, False, INK),
    # "The whole lit overlap, tiled": windows, area, verdicts, held-out median of the accepted and
    # how many are under 3 px (the one that is not is the 316 px window, an inlier-ratio limit of
    # the metric - REPORT says so, and the deck must not imply 37/37 were all sub-pixel).
    # "Chandrayaan-2 OHRC -> LRO NAC": the SAC equatorial rows' held-out median range, ref_gsd_m.
    ("Held-out residual (not ground truth): the whole lit, textured overlap of one OHRC frame "
     "with one NAC at 74 °S, tiled, not hand-picked — 37 windows, 13.1 km² — accepts 37/37, "
     "median 0.61 px = 0.57 m on the 0.93 m grid, 36 of 37 under 3 px; SAC's equatorial pair, "
     "174° apart: 0.69–1.68 px (1.1–2.7 m), 1.62 m grid.", 1, False, INK),
    # "Sub-pixel accuracy, with the pixel grid named": the synthetic row (0° / 15°), the loop
    # row (px, m, grid), the MiLOI row (S1 median, n, grids) + the MiLOI table's S1 truth error.
    # Each figure names its grid AND its metres (Invariant 2), and says what it is measured
    # against. Loops are consistency, NOT ground accuracy - REPORT's loop section says so in
    # those words, and a semicolon after "vs exact truth" would let them read as truth error
    # (claim-checker M2/M3/L3, v4).
    ("Sub-pixel, grid named: 0.086 px = 5.1 m vs exact truth at 0° and 15° Sun azimuth "
     "difference (synthetic, 60 m grid); six loops close to a median 0.086 px = 0.107 m on the "
     "1.245 m NAC grid — consistency, not ground truth; MiLOI network truth, 9 S1 pairs: median "
     "0.52 px = 0.73 m on 1.1–1.5 m grids (truth error 0.22 px).", 1, False, INK),
    ...
    # MiLOI's 90° is a SUN-VECTOR angle; every other Sun figure on the deck is an azimuth
    # difference. Naming it stops a reader comparing it with the sweep's 153° (M4, v4).
    ("The hard band: no method registered any of the 16 MiLOI pairs with Sun vectors 90° or more "
     "apart (all scene S3); our sweep accepted 0 of 12 at 60–120° azimuth, 10 of 15 at 120–153°; "
    ...
    ("Archive geometry is not truth: at SAC's equatorial site the NAC's published corners sit "
     "1.9 km from the OHRC grid; a wide search removes it first.", 1, False, INK),
    ...
    ("Relief parallax is not a homography: TMC-2 fore vs aft (same sensor, same Sun) accepts 1 "
     "window of 4, held-out median 2.6 px (15.5 m) on its 5.9 m grid.", 1, False, INK),
    ("Strategies for overcoming these challenges", 0, True, BLUE),
    ("Tile-level local transforms and LOLA orthorectification for relief; full-scene tiled OHRC "
     "strips; IIRS band selection for the infrared leg.", 1, False, INK),
]
...
S5 = [
    ...
    ("Landing-site safety at the lunar south pole: a hazard map is only as good as the "
     "alignment beneath it. Every region carries its own verdict; on 22 windows with Sun azimuths "
     "under 10° apart, planted errors of 5 m or more were all flagged, 74% at 3 m, and at 2 m or "
     "less almost none — that floor is stated, not hidden (8 SAC windows with Sun azimuths "
     "132–174° apart: 0/16 false alarms, 84% at 5 m, 100% from 10 m). Planted rotation and "
...
SLIDES = {
    ...
    4: {"text": [(LEFT_COL, S4, 15, 12)],        # 12: the sub-pixel bullet with its metres (20 Sep) did not fit at 12.5
