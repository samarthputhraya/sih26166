...
Output   presentation/SIH26166_LunaXX_deck.pptx   (national round, v3 - 20 Sep 2026)

V3 (NATIONAL ROUND, after the 20 Sep audit). The text below is rebuilt around the real-data
evidence of 18-20 Sep (REPORT.md). Every number on a slide was copied from REPORT.md at the
evidence-freeze commit b678272 (FROZEN 20 Sep 04:27, 10/10 steps), and the comment beside it
...
S2 = [
    ...
    ("Planted wrong answers, 22 real OHRC/NAC windows (Sun azimuths under 10° apart): 0/44 "
     "false alarms; flagged 74% at 3 m, 100% from 5 m, 1/352 at 1–2 m; 8 SAC windows, Sun "
     "azimuths 132–174° apart: 0/16 false alarms, 94% at 5 m, 100% from 10 m. No matches means "
...
S3_LEFT = [
    ...
    ("Hardware: a standard laptop, CPU only, no discrete GPU, fully offline; median 10.7 s per "
...
S4 = [
    ...
    ("Built and measured: 136 real window pairs, 8 instrument pairings, public data, open-source "
     "libraries; one command re-runs every number from a clean commit.", 1, False, INK),
    # "Chandrayaan-2 OHRC -> LRO NAC": held-out median range, Δsun az range, ref_gsd_m values.
    ("Held-out residual (not ground truth), OHRC → NAC, median per window: 0.41–1.01 px "
     "(0.51–0.94 m) at 74 °S, Sun azimuths 3–6° apart, ~1 m grids; 0.69–1.68 px (1.1–2.7 m) on "
     "SAC's equatorial pair, 174° apart, 1.62 m grid.", 1, False, INK),
    ...
    ("Sub-pixel, grid named: 0.086 px = 5.1 m vs exact truth at 0° and 15° Sun azimuth "
     "difference (synthetic, 60 m grid); six loops close to 0.08 px (0.10 m) on the 1.245 m NAC "
     "grid; MiLOI network truth, 9 S1 pairs: median 0.52 px on 1.1–1.5 m grids (truth error "
     "0.22 px).", 1, False, INK),
    ...
    ("Archive geometry is not truth: at SAC's equatorial site the NAC's published corners sit "
     "1.9 km from the OHRC grid; a wide search removes it before matching.", 1, False, INK),
...
S5 = [
    ...
    ("Landing-site safety at the lunar south pole: a hazard map is only as good as the "
     ...
     "132–174° apart: 0/16 false alarms, 94% at 5 m, 100% from 10 m).", 1, False, INK),
